...
## OJS runtime 注入の暗黙仕様 (sister-project の発見)

`mechanics-class-simulation` で発見された未文書の挙動 (上流 r-wasm/quarto-live にも未報告):

> **Quarto Live filter は `{ojs}` セルが 1 つも無いページに OJS runtime を inject しない。**
> その結果 `{pyodide}` セルの起動に必要な runtime が読み込まれず、Pyodide が空のまま固まる。

### 症状

- `{pyodide}` だけのページ (スライダーを `Inputs.range` で持たず、`FuncAnimation` で完結させた場合など) で
  `Loading...` のまま 30 秒経っても描画されない
- ブラウザ Console に `monaco` / `pyodide-worker` 関連のメッセージは出るが `runPython` が呼ばれない

### 回避策: ダミー `{ojs}` セル

ページ末尾 (または冒頭) に以下を 1 個置くだけで OJS runtime が注入され、`{pyodide}` セルが正常起動する:
...
ポイント:
- **代入式にする** (`1` だけだと "1" が画面に表示される)
- `output: false` を付けて結果を非表示
- `echo: false` でソース行も隠す

### いつ必要か

- ページに `Inputs.range` / `Inputs.checkbox` 等の OJS UI が **1 つも無い**
- かつ `{pyodide}` セルがある

スライダー駆動の対話デモ (`Inputs.range` を使う) には不要 (既に `{ojs}` セルが存在するため)。
